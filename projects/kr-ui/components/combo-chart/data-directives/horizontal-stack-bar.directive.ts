...
export class KruiChartHorizontalStackBarDirective implements OnInit,
  ...
  @Input() public workgroundPadding: KruiChartWorkgroundPadding = { top: 10, right: 5, left: 2, bottom: 10 };
  ...
  private renderBars(
    ...
    items
      ...
      .attr('x', (_, i) => {
        ...
        const previousValue = this._dataValueMap.get(-xCounter - 1)?.calculated[i];
        if (this._previousStackedData?.length && previousValue) {
          return notNul(secondaryAxis(previousValue[0]));
        }
    ...
    (items.selectAll('rect.krui-chart-data-bar') as d3.Selection<SVGRectElement, KruiChartBarData, SVGRectElement, unknown>)
      ...
      .attr('x', d => notNul(secondaryAxis(d[0])))
      .attr('width', d => {
    ...
    if (this.showValues) {
      ...
      items
        ...
        .attr('x', (_, i) => {
          ...
          if (this._previousStackedData?.length && previousValue) {
            return getBarHorizontalTextPositionStack(this.valuePosition, this.valuesFontSizePx, secondaryAxis, [
              previousValue[1], previousValue[0], previousValue[2],
            ]);
          }
          return secondaryAxis.range()[0];
        })
  ...
  private scaleBars(
    ...
    if (secondaryAxisEnable) {
      (this._target.selectAll(`.krui-chart-data-bar`) as d3.Selection<SVGRectElement, KruiChartBarData, SVGRectElement, unknown>)
        .transition()
        .duration(animationDuration)
        .attr('x', d => notNul(barConfig.secondaryAxis(d[0])))
        .attr('width', d => {
  ...
  private getBarsConfigs(
    ...
    if (type === 'enum') {
      ...
      barWidth = groupSize > 0 ? (this._context.canvasHeight / this._maxDataLength) : 0;
      barWidthCalculated = barWidth * this._barWidthPercent / 100;
      barPadding = barWidth * (1 - this._barWidthPercent / 100) / 2;
    }
