...
export class KruiChartHorizontalBarDirective implements OnInit,
  ...
  @Input() public workgroundPadding: KruiChartWorkgroundPadding = { top: 0, right: 5, left: 0, bottom: 0 };
  ...
  public getWorkgroundPadding(axisType: KruiChartAxisType): KruiChartWorkgroundPadding {
    if (this._initialData?.length) {
      const y = axisType === 'enum' ? 0 : (this.wrap.workGroundHeight / (this._data?.length ?? 10)) / 2 ?? 0;
      return {
  ...
  public toggleState(
    ...
    if (hidden) {
      this._target
        ...
        .attr('width', secondaryAxis.range()[0]);
  ...
  public render(
    ...
    this._target = target;
    this._id = id;
    const primaryAxis = context.primaryAxis.d3Scale as KruiChartScale; // y
    const secondaryAxis = context.secondaryAxis.d3Scale as KruiChartScale; // x
  ...
  private renderBars(
        ...
        d => {
          return `translate(0, ${axisIsEnum(primaryAxis, type)
            ...
            primaryAxis(d[0]) - (this.fixedBarWidth ?
              ...
              barConfig.barWidthCalculated * this._zoomK)})`;
        })
    ...
    if (type === 'enum' || this.tooltipHoverEffect) {
      const tooltip = bars
        ...
        .attr('width', secondaryAxis.range()[1])
        .attr('opacity', '0.1')
    ...
    bars.select('rect.krui-chart-data-bar')
      .transition()
    ...
    if (this.showValues) {
      bars
        ...
        .attr('x', (_, i) => {
          if (!!this._previousData?.length && this._previousData[i]) {
            return getBarHorizontalTextPosition(this.valuePosition, this.valuesFontSizePx, secondaryAxis, this._previousData[i][1]);
          }
          return -this.valuesFontSizePx;
        })
        .attr('y', () => {
          if (axisIsEnum(secondaryAxis, type)) {
        ...
        .text((d) => {
          ...
          if (this.showEnumValues && axisIsEnum(secondaryAxis, type)) {
            return d[2];
          }
        ...
        .transition()
        .duration(this.animationType === 'none' ? 0 : this._tempDurationAnimation)
        .attr('x', d => getBarHorizontalTextPosition(this.valuePosition, this.valuesFontSizePx, secondaryAxis, d[1]));
    }
  ...
  private scaleBars(
    ...
    if (secondaryAxisEnable) {
      (this._target.selectAll(`.krui-chart-data-bar`) as d3.Selection<SVGRectElement, KruiChartBarData, SVGRectElement, unknown>)
        ...
        .attr('width', d => {
          if (!d[1]) return 0;
          const delta = notNul(barConfig.secondaryAxis(d[1])) - barConfig.secondaryAxis.range()[0];
    ...
    if (barConfig.type === 'enum' || this.tooltipHoverEffect) {
      this._target
        ...
        .attr('width', barConfig.secondaryAxis.range()[1]);
    ...
    if (this.showValues) {
      (this._target.selectAll(`.krui-chart-stack-text`) as d3.Selection<SVGTextElement, KruiChartBarData, SVGTextElement, unknown>)
        .transition()
        .duration(animationDuration)
        .attr('x', d => getBarHorizontalTextPosition(this.valuePosition, this.valuesFontSizePx, barConfig.secondaryAxis, d[1]))
        .attr('y', () => {
          if (barConfig.type === 'enum') {
            return (barConfig.barWidthCalculated + barConfig.barPadding) / 2;
          }
  ...
  private getBarsConfigs(
    ...
    let barPaddingPercent: number;

    if (type === 'enum') {
      barWidth = !this.state.get(0) ? (primaryAxis as ScaleBand<any>).bandwidth() : 0;
      barWidthPercent = barWidth * this._barWidthPercent / 100;
      barPaddingPercent = barWidth * (1 - this._barWidthPercent / 100);
    } else {
      barWidth = !this.state.get(0) ? this._context.canvasHeight / this._data.length : 0;
      barWidthPercent = barWidth * this._barWidthPercent / 100;
      barPaddingPercent = barWidth * (1 - this._barWidthPercent / 100) / 2;
    }
    this._bandwidth = barWidth;
    return {
      type,
      barWidth,
      barWidthCalculated: barWidthPercent,
      barPadding: barPaddingPercent,
      primaryAxis,
      secondaryAxis,
    };
  }
