...
import { getBarHorizontalTextPositionStack } from '../constants/inner/bar.pure';
import { KruiChartStackedService } from '../services';


@Directive({
  selector: '[kruiChartHorizontalRangeBar]',
  exportAs: 'kruiChartHorizontalRangeBar',
  providers: [
    {
      provide: KruiChartDataLayerColorProvider,
      useExisting: KruiChartHorizontalRangeBarDirective,
    },
    {
...
export class KruiChartHorizontalRangeBarDirective implements OnInit,
  OnChanges,
  KruiChartDataLayerColorProvider,
  KruiChartDataLayerValueShower,
  KruiChartDataLayerAnimated,
  ...
  private renderBars(
    ...
    items
      ...
      .attr('x', (_, i) => {
        ...
        if (xCounter === barConfig.activeCount) {
          xCounter = 0;
        }
        if (!!this._previousData?.length && this._previousData[xCounter]?.[1]) {
          return notNul(secondaryAxis(this._previousData[xCounter][1][i][0]));
        }
      ...
      .attr('width', (_, i) => {
        ...
        if (!!this._previousData?.length && this._previousData[widthCounter]?.[1]) {
          const delta = notNul(secondaryAxis(this._previousData[widthCounter][1][i][1])) - notNul(secondaryAxis(this._previousData[widthCounter][1][i][0]));
          return delta > 0 ? delta : notNul(secondaryAxis(this._previousData[widthCounter][1][i][1]));
        }
        return 0;
      })
    ...
    (items.selectAll('rect.krui-chart-data-bar') as d3.Selection<SVGRectElement, [number, number], SVGRectElement, unknown>)
      ...
      .attr('x', d => {
        return notNul(secondaryAxis(d[0]));
      })
      .attr('width', (d) => {
        if (!d) return 0;
        const delta = notNul(secondaryAxis(d[1])) - notNul(secondaryAxis(d[0]));
        return delta > 0 ? delta : notNul(secondaryAxis(d[0]));
      });
      ...
      items
        ...
        .attr('x', (d, i) => {
          ...
          if (!!this._previousData?.length && this._previousData[xCounterText]?.[1]) {
            return getBarHorizontalTextPositionStack(this.valuePosition, this.valuesFontSizePx, secondaryAxis, this._previousData[xCounterText][1][i]);
          }
          return secondaryAxis.range()[0];
        })
        .attr('y', (_, i) => {
        ...
        .transition()
        .duration(this.animationType === 'none' ? 0 : this._tempDurationAnimation)
        .attr('x', d => getBarHorizontalTextPositionStack(this.valuePosition, this.valuesFontSizePx, secondaryAxis, d));
    }
  ...
  private scaleBars(
    ...
    if (secondaryAxisEnable) {
      (this._target.selectAll(`.krui-chart-data-bar`) as d3.Selection<SVGRectElement, [number, number], SVGRectElement, unknown>)
        ...
        .duration(animationDuration)
        .attr('x', d => {
          return notNul(barConfig.secondaryAxis(d[0]));
        })
        .attr('width', (d) => {
          if (!d) return 0;
          const delta = notNul(barConfig.secondaryAxis(d[1])) - notNul(barConfig.secondaryAxis(d[0]));
    ...
    if (this.showValues) {
      (this._target.selectAll(`.krui-chart-stack`)
        ...
        .attr('x', d => getBarHorizontalTextPositionStack(this.valuePosition, this.valuesFontSizePx, barConfig.secondaryAxis, d));
    }
  }
