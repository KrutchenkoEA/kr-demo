...
export class KruiChartComboBarDirective implements OnInit,
  ...
  private renderBars(
    ...
    items
      ...
      .attr('y', (_, i) => {
        ...
        if (!!this._previousData?.length && this._previousData[yCounter]?.[1]) {
          return notNul(secondaryAxis(this._previousData[yCounter][1][i]));
        }
      ...
      .attr('height', (_, i) => {
        ...
        if (!!this._previousData?.length && this._previousData[heightCounter]?.[1]) {
          const delta = secondaryAxis.range()[0] - notNul(secondaryAxis(this._previousData[heightCounter][1][i]));
          return delta > 0 ? delta : notNul(secondaryAxis(this._previousData[heightCounter][1][i]));
        }
        return 0;
      })
      .attr('width', () => {
    ...
    (items.selectAll('rect.krui-chart-data-bar') as d3.Selection<SVGRectElement, number, SVGRectElement, unknown>)
      ...
      .attr('y', d => notNul(secondaryAxis(d)))
      .attr('height', (d) => {
        if (!d) return 0;
      ...
      items
        ...
        .attr('y', (d, i) => {
          ...
          if (!!this._previousData?.length && this._previousData[yCounterText]?.[1]) {
            return getBarTextPosition(this.valuePosition, this.valuesFontSizePx, secondaryAxis, this._previousData[yCounterText][1][i]);
          }
          return secondaryAxis.range()[0];
        })
        .text((d) => {
        ...
        .transition()
        .duration(this.animationType === 'none' ? 0 : this._tempDurationAnimation)
        .attr('y', d => getBarTextPosition(this.valuePosition, this.valuesFontSizePx, secondaryAxis, d));
    }
  ...
  private scaleBars(
    ...
    if (primaryAxisEnable) {
      (this._target.selectAll(`.krui-chart-stack`)
        .selectAll(`.krui-chart-data-bar`) as d3.Selection<SVGRectElement, number, SVGRectElement, unknown>)
        .transition()
    ...
    if (secondaryAxisEnable) {
      (this._target.selectAll(`.krui-chart-data-bar`) as d3.Selection<SVGRectElement, number, SVGRectElement, unknown>)
        .transition()
        .duration(animationDuration)
        .attr('y', d => notNul(barConfig.secondaryAxis(d)))
        .attr('height', (d) => {
          if (!d) return 0;
          const delta = barConfig.secondaryAxis.range()[0] - notNul(barConfig.secondaryAxis(d));
    ...
    if (this.showValues) {
      (this._target.selectAll(`.krui-chart-stack`)
        .selectAll(`.krui-chart-stack-text`) as d3.Selection<SVGRectElement, number, SVGRectElement, unknown>)
        .transition()
        .duration(animationDuration)
        .attr('x', (_, i) => {
          const pos = (barConfig.barWidthCalculated + 2 * barConfig.barPadding) * i + barConfig.barWidthCalculated / 2 + barConfig.barPadding + barConfig.stackPadding;
        ...
        .attr('y', d => getBarTextPosition(this.valuePosition, this.valuesFontSizePx, barConfig.secondaryAxis, d));
    }
  }
